...
export const goalController = {
  ...
  async propose(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!drafted?.length) throw badRequest('Could not break that down into goals just now');
      const n = await save(req.user.id, input.intent, input.exchangeId ?? null, drafted);
      await refresh(req.user.id);
